alter table public.dias_sala_profetica
  add column eh_teste boolean not null default false;

comment on column public.dias_sala_profetica.eh_teste is
  'Dia aberto só para testes: aceita inscrições, mas não conta para a regra de uma participação por conferência.';

insert into public.dias_sala_profetica (dia, abre_as, eh_teste) values ('2026-09-24', '00:00', true);
...
begin
  ...
  if v_contato_id is not null then
    ...
    -- dias de teste não contam, nem para bloquear nem para serem bloqueados.
