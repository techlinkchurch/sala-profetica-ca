create table public.dias_sala_profetica (
  dia date primary key,
  abre_as time not null default '00:00',
  criado_em timestamptz not null default now()
);

comment on table public.dias_sala_profetica is
  'Dias da conferência atual em que a Sala Profética aceita inscrições. abre_as é no horário de Belém. Quem já tem inscrição em outro dia desta tabela só pode se inscrever de novo se o status for nao_compareceu.';

alter table public.dias_sala_profetica enable row level security;

create policy dias_sala_select_staff on public.dias_sala_profetica
  for select to authenticated using (is_staff());
create policy dias_sala_insert_staff on public.dias_sala_profetica
  for insert to authenticated with check (is_staff());
create policy dias_sala_update_staff on public.dias_sala_profetica
  for update to authenticated using (is_staff()) with check (is_staff());
create policy dias_sala_delete_staff on public.dias_sala_profetica
  for delete to authenticated using (is_staff());

insert into public.dias_sala_profetica (dia, abre_as) values
  ('2026-09-25', '00:00'),
  ('2026-09-26', '08:50');

update public.config_sala set vagas_dia = 50, atualizado_em = now() where id = 1;
...
begin
  ...
  select abre_as into v_abre_as from dias_sala_profetica where dia = v_dia;
  if not found then
  ...
  if v_contato_id is not null then
    ...
    -- uma participação por conferência; só quem não compareceu é liberado para outro dia
    select f.dia_evento into v_dia_anterior
    from fila_sala_profetica f
    join dias_sala_profetica d on d.dia = f.dia_evento
    where f.contato_id = v_contato_id and f.dia_evento <> v_dia and f.status <> 'nao_compareceu'
